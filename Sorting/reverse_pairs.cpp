...
ll merge(v_ll &nums, ll l, ll m, ll r);
ll merge_sort(v_ll &nums, ll l, ll r);
...
int main(){
    ...
    while(T){
        ...
        ll res = merge_sort(nums,0,nums.size()-1);
        deb(res);
        nums.clear();
        T--;
    }
...
ll merge_sort(v_ll &nums, ll l, ll r) {
    if (l >= r)
        return 0;
    ll m = (l + r) >> 1;
	ll left = merge_sort(nums, l, m);
    ll right = merge_sort(nums, m + 1, r);
    ll center = merge(nums, l, m, r);
