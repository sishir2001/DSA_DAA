// find the maximum profits by buying and selling stocks 
#include<bits/stdc++.h>
using namespace std;

typedef long long ll;
// typedef vector<ll> v_ll;

#define mod 1000000007 // 1e9+7
#define deb(x) cout << #x << " = "<<x<<"\n" // for debugging
#define deb2(x,y) cout << #x << " = "<<x << " , " << #y << " = " << y <<"\n" // for debugging 
#define FOR(a,end) for(int (a) = 0;(a) < (end);(a)++) // regular for loop
#define FOR_desc(a,end) for(int (a) = (end)-1;(a) >= 0;(a)--) // regular for loop decreasing
#define FOR_custom_less(a,end,start) for(int (a) = (start);(a) < (end);(a)++) // regular for loop
#define FOR_custom_less_or_equal(a,end,start) for(int (a) = (start);(a) <= (end);(a)++) // regular for loop

int stockBuyAndSell(int *a,int n){
    // @param a : pointer to a heap array
    // @param n : size of the array
    // * T(n) = O(n)
    // * S(n) = O(1) auxilliary space 

    int l =0,h=0,profit = 0; // l : index of lowest price , h : index of the highest price 
    FOR_custom_less(i,n,1){
        if(a[h] > a[i]){
            // * sell the stock
            profit += a[h]-a[l];
            h = i;
            l = i;
        }
        else if(a[h] < a[i]){
            h = i;
        }
    }
    profit += a[h] - a[l];
    return profit;
}

int main(){
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    cout.tie(NULL);
    int T,n;
    cin >> T;
    while(T){
        cin >> n;
        int *a = new int[n];
        FOR(i,n){
            cin >> a[i];
        }
        cout << stockBuyAndSell(a,n) << "\n";
        delete[] a;
        T--;
    }
    return 0;
}
