...
void topological_sort_bfs(vector<v_int> &adj);
...
void topological_sort_bfs(vector<v_int> &adj){
	...
	int V = adj.size();
	// * counting indegree for each job
	vector<int> indegree(V,0);
	// *traversing the graph for counting indegree
	FOR(i,V,0){
		for(auto v : adj[i]){
			indegree[v]++;
		}
	}
	// *pushing all the 0 indegree nodes to queue
	queue<int> q;
	FOR(i,V,0){
		if(indegree[i] == 0)
			q.push(i);
